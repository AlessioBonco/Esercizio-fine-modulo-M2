...
//PARTE 2:
      function cercaLavoro() {
        ...
        console.log("Titolo cercato: " + titolo);
        console.log("Posizione cercata: " + posizione);

        let risultati = searchJobs(titolo, posizione);

        console.log("Risultati trovati: ", risultati);

        let listaRisultati = document.getElementById('risultati');
        listaRisultati.innerHTML = '';  // Resetta i risultati precedenti

        if (risultati.count > 0) {
            risultati.result.forEach(job => {
                let li = document.createElement('li');
                li.textContent = "Titolo: " + job.title  + " Posizione " + job.location ;
  ...
  // Esempio di array di inserzioni di lavoro
  const jobs = [
  ...
  let searchResult = searchJobs("designer", "ny");
